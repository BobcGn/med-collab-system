...
class PatientService(
     ...
     * 3. 创建患者记录
     */
    suspend fun createPatient(patient: PatientDto.CreatePatient): PatientDto.PatientInfo {
        // 检查患者是否已存在
        ...
        if (!authClient.validateDoctor(patient.attendingDoctorId)) {
            throw PatientException.DoctorIdInvalidException()
        }

        // 验证医院ID是否有效
        if (authClient.getUserHospitalId(patient.attendingDoctorId) != patient.hospitalId) {
     ...
     * 获取患者详情
     */
    suspend fun getPatientInfo(patientId: String): PatientDto.PatientInfo {
        return patientRepository.findPatientById(patientId)
     ...
     * 根据医院和病历号获取患者
     */
    suspend fun getPatientByHospitalAndPatientId(hospitalId: String, patientId: String): PatientDto.PatientInfo {
        return patientRepository.findPatientByHospitalAndPatientId(hospitalId, patientId)
            ?: throw PatientException.PatientNotFoundException()
    }
     ...
     * 获取医院患者列表（分页）
     */
    suspend fun getHospitalPatients(hospitalId: String, page: Int = 0, size: Int = 20): List<PatientDto.PatientListItem> {
        return patientRepository.findPatientsByHospital(hospitalId, page, size)
    }

    /**
     * 获取科室患者列表（分页）
     */
    suspend fun getDepartmentPatients(department: String, page: Int = 0, size: Int = 20): List<PatientDto.PatientListItem> {
        return patientRepository.findPatientsByDepartment(department, page, size)
    }

    /**
     * 获取医生负责的患者列表（分页）
     */
    suspend fun getDoctorPatients(doctorId: String, page: Int = 0, size: Int = 20): List<PatientDto.PatientListItem> {
        return patientRepository.findPatientsByAttendingDoctor(doctorId, page, size)
    }
    ...
    suspend fun searchPatients(keyword: String, page: Int = 0, size: Int = 20): List<PatientDto.PatientListItem> {
        return patientRepository.searchPatients(keyword, page, size)
    }
    ...
    suspend fun getRecentPatients(days: Int = 7, limit: Int = 20): List<PatientDto.PatientListItem> {
        return patientRepository.getRecentPatients(days, limit)
    }
    ...
    suspend fun getPatientStatistics(hospitalId: String? = null): PatientDto.PatientStatistics {
        return patientRepository.getPatientStatistics(hospitalId)
    }
     ...
     * 更新患者状态（出院、死亡等）
     */
    suspend fun updatePatientStatus(patientId: String, status: Status, operatorId: String): Boolean {
        // 验证患者是否存在
        val patient = patientRepository.findPatientById(patientId)
            ?: throw PatientException.PatientNotFoundException()

        // 权限验证 - 只有主治医生或管理员可以修改患者状态
        if (!authClient.canModifyPatient(operatorId, patient.attendingDoctorId)) {
            throw PatientException.PermissionDeniedException()
        }
    ...
    suspend fun transferPatient(
        ...
        if (!authClient.validateDoctor(newAttendingDoctorId)) {
            throw PatientException.DoctorIdInvalidException()
        }
